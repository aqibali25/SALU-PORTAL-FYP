...
const AdmissionSchedule = () => {
  const admissionScheduleCards = [
    {
      id: 1,
      title: "Add Admission Schedule",
      bgColor: "#FFFBEB",
      borderColor: "#FACC15",
      iconBg: "#FACC15",
    ...
    {
      id: 2,
      title: "View Admission Schedules",
      bgColor: "#EFF6FF",
      borderColor: "#3B82F6",
      iconBg: "#3B82F6",
      Icon: FaClipboardCheck,
  ...
  return (
    ...
    >
      {/* Integrated AdmissionScheduleCardsLayout directly */}
      <div className="flex flex-col gap-3 w-full min-h-[80vh] bg-[#D5BBE0] rounded-md !p-5">
        ...
        <div className="flex flex-wrap items-center justify-start gap-5 min-h-[60vh] w-full bg-white dark:bg-gray-900 rounded-md overflow-x-auto !p-5">
          {admissionScheduleCards.length === 0 ? (
            <h1 className="w-full text-center text-2xl text-red-600">
              You don't have any Admission Schedules created yet!
            </h1>
          ) : (
            admissionScheduleCards.map((card, index) => (
              <AdmissionCard
                key={index}
                title={card.title}
                ...
                iconBg={card.iconBg}
                Icon={card.Icon}
                to={`AdmissionSchedule/${card.title.replace(/\s+/g, "")}`}
